...
interface SignalWithScore extends RawSignalData {
...
function getDmiScore(data: string): IndicatorResult {
  ...
  if (adx < 15 && adxSlope <= 0) { confidence *= 0.2; }
  else if (adx < 15 && adxSlope > 0) { confidence *= 0.5; }
  else if (adx >= 15 && adx < 20 && adxSlope > 0) { confidence *= 0.7; }
  return { score: confidence * direction, confidence };
}
...
function getAgulhadaScore(data: string): IndicatorResult {
    ...
    const direction = signal > 0 ? 1 : -1;
    let confidence = 0;
    if (isCompacted === 1) { confidence = 0.70; }
...
async function getConvergenceBonus(ticker: string, currentTf: string, currentDirection: number): Promise<number> {
    ...
    // Lógica de hierarquia de timeframes
    ...
    const bonusPerTf = 0.10; // Bônus de 10% por timeframe alinhado
...
export async function processSignalBuffer(signalBuffer: SignalWithScore[]) {
  ...
  // Chamada assíncrona para o novo bônus de convergência
